...
//    m.changeScene("screens/settingsScreen.fxml");
	...
	private void drawTiles() {
		...
		for (GraphicTile gt : gtiles) {
			if (!gt.isVisiblee() || gt.toDraw()) {
				...
				Tile exchangeTile = new Tile(gt.getLetter().getText().charAt(0), gt.getTile().getValue(),
						Tilestatus.INBAG);
					...
					if (b.contains(e.getX(), e.getY())) {
						...
						for (int k = 0; k < 7; k++) {
							if (gtiles[k].isHighlighted()) {
								...
								Tile newTile = new Tile(gtiles[k].getLetter().getText().charAt(0),
										gtiles[k].getTile().getValue());
								newTile.setXY(i, j);
								int ite = 0;
							...
							public void run() {
								...
								if (match.isOver()) {
									...
									try {
										...
										System.err.println("The game could not stop properly");
									}
	...
	public void endOfGame() throws IOException {
		if (this.thisPlayer == gameInformation.getHost()) {
			gameInformation.getServermatch().getServer().stopServer();
		} else {
			gameInformation.getClientmatch().getProtocol()
					.sendToServer(new DisconnectMessage(this.thisPlayer.getName()));
		}
		Main m = new Main();
