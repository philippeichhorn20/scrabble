...
public class GameScreenController extends Thread {
	...
	private Label[] history = new Label[11];// =
											// {history1,history2,history3,history4,history5,history6,history7,history8,history9,history10,history11};
	...
	public void endTurnB() {
		...
		if (valid) {
			drawTiles();
			newHistoryMessage(Main.profile.getName().substring(0, 1).toUpperCase()
					+ Main.profile.getName().substring(1).toLowerCase() + " finished his turn");
			// resetTilesButton.setVisible(false);
			turn++;
		} else {
			TutorialScreenController.AlertBox.display("Not a valid word!", "This word is not in our dictionary!");
	...
	private void drawTiles() {
		...
		for (GraphicTile gt : gtiles) {
			if (!gt.isVisiblee() || gt.toDraw()) {
				// Tile newTile = servMatch.getTileBag().drawTile(); unlock when servermatch is
				// done
					...
					if (b.contains(e.getX(), e.getY())) {
						...
						for (int k = 0; k < 7; k++) {
							if (gtiles[k].isHighlighted()) {
								...
								Rectangle rec = new Rectangle(gtiles[k].getRec().getX(), gtiles[k].getRec().getY(),
										gtiles[k].getRec().getWidth(), gtiles[k].getRec().getHeight());
								rec.setFill(Color.web("#ffe5b4"));
								Text let = new Text(gtiles[k].getX(), gtiles[k].getY(),
										gtiles[k].getLetter().getText());
								...
								newHistoryMessage(GameInformation.getInstance().getClientmatch().getCurrentPlayerName()
										.substring(0, 1).toUpperCase()
										+ GameInformation.getInstance().getClientmatch().getCurrentPlayerName()
												.substring(1).toLowerCase()
										+ " placed a tile " + newTile.getLetter() + " on " + newTile.getX() + "  "
										+ newTile.getY());
								GameInformation.getInstance().getClientmatch().getScrabbleBoard().placeTile(newTile,
	...
	public void resetTilesAction() {
		...
		newHistoryMessage(
				GameInformation.getInstance().getClientmatch().getCurrentPlayerName().substring(0, 1).toUpperCase()
						+ GameInformation.getInstance().getClientmatch().getCurrentPlayerName().substring(1)
								.toLowerCase()
						+ " returned his tiles to the rack");
	...
	public void setUp(MouseEvent e) {
		...
		if (!setUpDone) {
				...
				public void run() {
					while (!GameInformation.getInstance().getClientmatch().isOver()) { // for (int i = 0;i<100;i++){
						...
						Platform.runLater(new Runnable() {
							@Override
							public void run() {
								currPlayerText.setText(GameInformation.getInstance().getClientmatch()
										.getCurrentPlayerName().substring(0, 1).toUpperCase()
										+ GameInformation.getInstance().getClientmatch().getCurrentPlayerName()
												.substring(1).toLowerCase());
								nameScore1.setText(
								...
								if (GameInformation.getInstance().getClientmatch().hasNewTiles()) {
									...
									for (int x = 0; x < newTiles.length; x++) {
										System.out.println(newTiles[x].getX() + "   " + newTiles[x].getY());
										placeTile(newTiles[x]);
									}
									match.dropNewTiles();
								}
								if (GameInformation.getInstance().getClientmatch().getProtocol().messageChanged()) {
									newHistoryMessageOther(GameInformation.getInstance().getClientmatch().getProtocol()
											.getHistoryMessage());
			...
			if (players[0] != null) {
				name1.setText(players[0].getName().substring(0, 1).toUpperCase()
	...
	public static void endOfGame() throws IOException {
		Main m = new Main();
		m.changeScene("screens/lobbyScreen.fxmll");
	...
	public static class AlertBox {
		public static void display(String title, String message) {
			...
			Button buttonLobby = new Button("To lobby");
			buttonLobby.setOnAction(e -> {
				try {
					endOfGame();
				} catch (IOException e1) {
					System.err.println("Could not exit to Lobby Screen");
					e1.printStackTrace();
				}
			});
